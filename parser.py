...
class PathConfig:
    ...
    def __init__(self, dir_archive):
        ...
        self.example_file_output_tweets = self.create_path_for_file_output_tweets(
            year=2020, month=12, day=1, hour=23, minute=00
        )

    def create_path_for_file_output_tweets(self, year, month, day, hour, minute) -> str:
        ...
        return os.path.join(
            self.dir_output,
            f"{year:04}-{month:02}-{day:02}-{hour:02}-{minute:02}.md",
        )
    ...
    def create_path_for_file_output_dms(
        ...
        return os.path.join(
            self.dir_output, kind, f"{kind}-{name}{index_suffix}.{file_format}"
        )

    def create_path_for_file_output_single(self, file_format: str, kind: str) -> str:
        """Builds the path for a single output file which, i.e. one that is not part of a larger group or sequence."""
        return os.path.join(self.dir_output, f"{kind}.{file_format}")
...
def get_twitter_api_guest_token(session, bearer_token):
    ...
    if not guest_token:
        raise Exception("Failed to retrieve guest token")
    return guest_token
...
def lookup_users(user_ids, users):
    ...
    filtered_user_ids = [user_id for user_id in user_ids if user_id not in users]
...
def convert_tweet(tweet, username, media_sources, users: dict, paths: PathConfig):
    ...
    body_markdown = (
        header_markdown + body_markdown + f"[{timestamp_str}]({original_tweet_url})"
    )
    ...
    return timestamp, body_markdown
...
def parse_tweets(username, users, paths: PathConfig):
    ...
    for tweets_js_filename in paths.files_input_tweets:
        json_content = read_json_from_js_file(tweets_js_filename)
    ...
    for timestamp, md in tweets:
        # Use a (markdown) filename that can be imported into Jekyll: YYYY-MM-DD-your-title-here.md
        dt = datetime.datetime.fromtimestamp(timestamp)
        grouped_tweets[(dt.year, dt.month, dt.day, dt.hour, dt.minute)].append(md)

    # for (year, month, day), content in grouped_tweets.items():
    for (year, month, day, hour, minute), content in grouped_tweets.items():
        # Write into *.md files
...
"""
        ...
        # md_string = content[0]
        md_path = paths.create_path_for_file_output_tweets(
    ...
    print(
        # f"Wrote {len(tweets)} tweets to *.md and *.html, "
        f"Wrote {len(tweets)} tweets to *.md, "
...
def parse_followings(users, user_id_url_template, paths: PathConfig):
    ...
    following_output_path = paths.create_path_for_file_output_single(
        file_format="txt", kind="following"
    )
...
def parse_followers(users, user_id_url_template, paths: PathConfig):
    ...
    followers_output_path = paths.create_path_for_file_output_single(
        file_format="txt", kind="followers"
    )
...
def parse_direct_messages(username, users, user_id_url_template, paths: PathConfig):
    ...
    for other_user_id, messages in conversations_messages.items():
        ...
        if len(messages) > 1000:
            ...
            conversation_output_path = paths.create_path_for_file_output_dms(
                name=other_user_short_name, file_format="md"
            )
...
def parse_group_direct_messages(username, users, user_id_url_template, paths):
    ...
    for conversation_id, messages in group_conversations_messages.items():
        ...
        if len(messages) > 1000:
            ...
            markdown += "\n\n----\n\n".join(md for _, md in messages)
            conversation_output_filename = paths.create_path_for_file_output_dms(
                name=group_name, file_format="md", kind="DMs-Group"
            )
...
def main():
    ...
    if not os.path.isfile(paths.file_tweet_icon):
        shutil.copy(f"{archive_path}/assets/images/favicon.ico", paths.file_tweet_icon)

    media_sources = parse_tweets(username, users, paths)
    ...
    if len(unknown_follower_user_ids) > 5000:
        ...
        print(
            "For some user IDs, the @handle is not included in the archive data"
            "Unknown user handles can be looked up online."
        ...
        if not get_consent(
            "Do you want to include handles of your followers"
            "in the online lookup of user handles anyway?",
            default_to_yes=True,
        ):
    ...
    print("before starting the download.\n")
